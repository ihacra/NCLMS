...
<body>
<form:form id="inputForm" modelAttribute="franchiseAdmin" action="${ctx}/admin/franchise/auditSave" method="post" cssClass="sui-form form-horizontal sui-validate">
    <form:hidden path="id"/>
    <div class="main-con">
        <br/>
        <div class="control-group">
            <div class="controls">加盟人姓名：${franchiseAdmin.name}</div>
        </div>
        <div class="control-group">
            <div class="controls">户籍所在地：${franchiseAdmin.address}</div>
        </div>
        <div class="control-group">
            <div class="controls">联系方式：${franchiseAdmin.mobile}</div>
        </div>
        <div class="control-group">
            <div class="controls">从业经验：${franchiseAdmin.experience}</div>
        </div>
        <div class="control-group">
            <div class="controls">投资金额：${franchiseAdmin.amount}(万元)</div>
        </div>
        <div class="control-group">
            <div class="controls">加盟地区：${franchiseAdmin.area}</div>
        </div>
        <div class="control-group">
            <div class="controls">客户数量：${franchiseAdmin.customer}</div>
        </div>
        <div class="control-group">
            <div class="controls">日均发件量：${franchiseAdmin.quantity}</div>
        </div>
        <div class="control-group">
            <label class="control-label" style="width: auto">审核结果：</label>
            <div class="controls">
                <form:radiobutton path="status" value="通过"/>通过&nbsp;&nbsp;&nbsp;
                <form:radiobutton path="status" value="驳回" checked="true"/>驳回
            </div>
        </div>
        <div class="control-group">
            <label for="remarks" class="control-label" style="width: auto">审核意见：</label>
            <div class="controls">
                <form:textarea path="remarks" maxlength="200" placeholder="请输入审核意见" rows="2" cssClass="input-large" data-rules="required"/>
            </div>
        </div>
